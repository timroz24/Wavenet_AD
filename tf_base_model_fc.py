...
#                         print(placeholder_name, data.shape)
                ...
                [val_loss,val_summary_loss] = self.session.run(
                    fetches=[self.loss, self.merged_summary_all],
                ...
                train_loss, _, train_summary_loss  = self.session.run(
                    fetches=[self.loss, self.step, self.merged_summary_all],
                ...
                if step % self.log_interval == 0:                    
                    ...
                    self.train_writer.add_summary(train_summary_loss, step)
                    self.val_writer.add_summary(val_summary_loss, step)
    ...
    def build_graph(self):
        with tf.Graph().as_default() as graph:
            ...
            self.merged_summary_all = tf.summary.merge_all()
