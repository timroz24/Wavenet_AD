...
def sequence_smae(y, y_hat, sequence_lengths):
    max_sequence_length = tf.shape(y)[1]
    y = tf.cast(y, tf.float32)
    smae = tf.abs(y_hat - y) 
    # ignore discontinuity
    zero_loss = tf.ones_like(smae)
    nonzero_loss = smae
    smae = tf.where(tf.logical_or(tf.equal(y, 0.0), tf.equal(y_hat, 0.0)), zero_loss, nonzero_loss)
...
#     sequence_mask = sequence_mask*(1 - is_nan)
    avg_smae = tf.reduce_sum(smae*sequence_mask) / tf.reduce_sum(sequence_mask)
    return avg_smae    
    
def sequence_smape_original(y, y_hat, sequence_lengths, is_nan):
    max_sequence_length = tf.shape(y)[1]
    y = tf.cast(y, tf.float32)
...
def sequence_smape_new(y, y_hat, sequence_lengths):
    print(get_tensor_shape(y))
    max_sequence_length = tf.shape(y)[1]
    y = tf.cast(y, tf.float32)
    smape = 2*(tf.abs(y_hat - y) / (tf.abs(y) + tf.abs(y_hat)))
    avg_smape = tf.reduce_sum(smape) / tf.cast(get_tensor_shape(y), tf.float32)
...
def get_target_data_cols(s, t):
    s = set(s)
    t = set(t)
    data_cols = list(s - t)
    target_sensors = list(t)
    print(data_cols)
    print(target_sensors)
    print(len(target_sensors))
    all_cols = target_sensors + data_cols
    return all_cols, len(target_sensors)

def find_missing_timesteps_idx_per_equipment(equipment_id, df_tmp, resample='1Min'):
    df_tmp = df_tmp.resample(resample).mean()
    tmp_len = df_tmp.shape[0]
    missing_idx = [-1]
    missing_idx.extend([i for i,flag in enumerate(df_tmp[df_tmp.columns[0]].isnull().values) if (flag==True)])
    missing_idx.append(df_tmp.shape[0])
    return df_tmp.values, missing_idx

def find_good_idx_equipment(idx, window_size):
    return list(itertools.chain(*[list(np.arange(i+1,j-window_size+1)) for i,j  in zip(idx[:-1],idx[1:]) if ((j-i)>=window_size)]))
